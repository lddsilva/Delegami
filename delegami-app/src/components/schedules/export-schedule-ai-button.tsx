...
import { exportScheduleAiContext } from '@/modules/schedules/actions'

export function ExportScheduleAiButton({ projectId, label }: { projectId: string; label: string }) {
  ...
  async function handleClick() {
    setLoading(true)
    setError(null)
    const result = await exportScheduleAiContext(projectId)
    setLoading(false)
    ...
    a.href = url
    a.download = `${label.replace(/\s+/g, '-')}-cronograma-ia.json`
